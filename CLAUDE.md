...
xcodebuild test -workspace ambilighthue.xcworkspace -scheme ambilighthue -testPlan ambilighthuetests

# Run a single test (class or method)
xcodebuild test -workspace ambilighthue.xcworkspace -scheme ambilighthue \
...
- **Swift Package Manager** (in `ambilighthue.xcodeproj`): Mocker (HTTP mocking), pulled from `github.com/WeTransfer/Mocker.git`. The `pod 'Mocker'` line in the Podfile is intentionally commented out.

```bash
# Install/update the CocoaPods dependencies (Alamofire, ViewInspector)
pod install
pod update
```
...
- `ambilighthue/`: Main app source code
- `ambilighthueTests/`: Unit tests
- `Pods/`: CocoaPods dependencies (Alamofire, ViewInspector) — gitignored
- `*.xcworkspace`: Main workspace — **gitignored and regenerated by `pod install`**. The shared scheme (`ambilighthue`) lives in `ambilighthue.xcodeproj/xcshareddata/xcschemes/`, not the workspace.
- `Podfile`: CocoaPods config for tvOS 17.5+
- ⚠️ A stray `SessionFactory.swift` exists at the **repo root** — it is an outdated, unused copy. The file actually compiled into the app is `ambilighthue/SessionFactory.swift`. Edit the one under `ambilighthue/`.
